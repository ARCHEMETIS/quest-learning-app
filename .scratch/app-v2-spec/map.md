...
- **[รวบสเปกสุดท้ายเป็น spec.md](issues/11-assemble-spec.md) — ✅ ถึง Destination: `spec.md` ที่ root ของ app-v2-spec (11 sections) รวมทุกการตัดสินใจ + ลิงก์ asset 5 ไฟล์ + รายการ "เจ้าของต้องทำเอง" 25 ข้อ พร้อมส่งต่อ effort implement**
- [ออกแบบ schema Supabase](issues/15-supabase-schema.md) — 14 ตาราง + 3 view + bucket `payment-slips`; /stats เป็น VIEW รันสิทธิ owner (เปิด anon อ่านได้ไม่หลุด PII), payments RLS เจ้าของแถว+admin verify, `activity_log` (user/event/created_at) เก็บ timestamp ทุก event ตั้งแต่วันแรกเพื่อกราฟโต; asset `assets/supabase-schema.md`
- [แผน deploy และโครงโปรเจกต์บน Netlify](issues/16-deploy-plan.md) — repo `luiquest` (Vite+React / netlify functions + scheduled), `netlify.toml` cron pre-generate ตี 2–5 ไทย, env 5 ตัว (service_role/Gemini ห้ามขึ้น `VITE_`), site `luiquest.netlify.app`, เช็คลิสต์ Supabase + Google OAuth; asset `assets/deploy-plan.md`
- [เก็บเงินจริงใน MVP](issues/13-real-monetization.md) — freemium **39 บาท/เดือน** ผ่าน PromptPay QR ยืนยันกึ่ง manual (pending→submitted→verified/rejected); premium = หลายหัวข้อ + แชทไม่จำกัด + badge; paywall ไม่ขวาง onboarding โผล่เฉพาะตอนชนเพดาน; fast-follow Wave 2/สัปดาห์ 3; รายเดือน = verify ถี่ + auto-expire runway สั้น
- [โมเดลธุรกิจระดับ BMC](issues/08-business-model.md) — BMC ครบ 9 ช่อง; VP ควบ 2 pain (ไม่รู้เริ่มตรงไหน + เริ่มแล้วไม่รอด); **revenue = 39 บาท/เดือน PromptPay** (เจ้าของเคาะรายเดือน 12 ก.ค. — เดิมเดารายเทอม 99 แล้วเปลี่ยน); segment กว้าง + beachhead นศ.ไอที; conversion เป้า 2–5%
- [วิธีนับและโชว์ผู้ใช้จริง](issues/14-user-count-evidence.md) — รายงาน 2 ระดับ ชู activated (ทำเควส ≥1); เอาหน้า /stats สาธารณะ + screenshot Supabase; ต้องเก็บ timestamp ทุก event ตั้งแต่วันแรก → **/stats ถูกเพิ่มเข้า design brief #10 แล้ว (10 หน้า)**
- [แผนหาผู้ใช้เดือนแรก](issues/09-growth-plan.md) — เริ่มเพื่อนตรง → กลุ่มมหาลัย → TikTok (ออปชัน); ทุกข้อความผูกลิงก์ชวน+การ์ดแชร์; milestone สัปดาห์ 1 (~20–40 สมัคร) / สัปดาห์ 2 (~50–80 + viral ชั้นสอง); เก็บตัวเลขไว้ pitch 9 ส.ค.
- [Design brief/พรอมต์ให้เพื่อนออกแบบ UI](issues/10-prototype-core-screens.md) — asset `assets/design-brief-ui.md`: brief เต็ม + พรอมต์ self-contained พร้อมวางใน Claude, **10 หน้า + app shell (รวม /stats แล้ว)** ครบ edge states; ดีไซน์เปิดกว้าง 100%
